...
// Check if the user is an admin and adjust the query accordingly
$is_admin = $_SESSION['is_admin']; // Assuming this session variable is set during login
$current_user_id = $_SESSION['user-id']; // ID of the current logged in user

if ($is_admin == 1) {
    // If the user is an admin, fetch all posts
    $query = "SELECT posts.id, posts.title, posts.category_id, users.username as author FROM posts JOIN users ON posts.author_id = users.id ORDER BY posts.id DESC";
} else {
    // If not an admin, fetch only posts from the logged-in user
